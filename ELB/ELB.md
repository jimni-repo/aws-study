...
## Health Check (상태 검사)
로드 밸런서에 연결된 인스턴스에 직접 트래픽을 발생시켜 인스턴스의 상태를 확인합니다. 대상 그룹의 상태 검사 기능을 통해 정상인 인스턴스로 라우팅합니다. 상태 검사의 상태는 `InService` 와 `OutService` 가 있습니다. 상태 검사 방식에는 Listen 하고 있는 포트의 상태를 검사하는 방법과 HTTP 와 HTTPS 를 통해 실제 HTML 에 접근하여 확인하는 방법이 있습니다.

## 추가적인 보안 그룹 설정
로드 밸런서 또한 네트워크 인터페이스 형태이기 때문에 보안 그룹을 통해 추가적인 보안 설정이 가능합니다. 인스턴스의 보안 그룹과 로드 밸런서의 보안 그룹을 통해 외부 공격에 대한 대비를 강력하게 할 수 있습니다. VPC 도 로드 밸런서와 인스턴스 그룹에 각각 구성이 가능한데, 이 구성은 VPC Peering, 라우팅 테이블 등 많은 추가적인 설정이 필요하다.

## 가용 영역 및 로드 밸런서 노드
로드 밸런서에서 가용 영역을 활성화 하면 해당 가용 영역에 로드 밸런서 노드를 생성합니다. 등록된 모든 가용 영역의 로드 밸런서가 트래픽을 수신하는 것이 아니라 활성화 된 가용 영역의 로드 밸런서만 라우팅됩니다.
...
### 고가용성 및 탄력성 (Autoscaling)
로드 밸런서는 클라이언트의 요청에 대한 부하에 맞게 자동적으로 리소스를 확장 및 축소하는 기능을 제공합니다. ELB 에 Auto Scaling Group(ASG) 를 결합하여 서비스할 수 있습니다. ASG 는 기본 적으로 서버의 전원이 켜져있는지만 확인하는 경우가 많은데, ELB 와 결합하면 좀 더 상세한 상태검사가 가능합니다. 이 구성을 위해서는 ASG 설저에서 상태 체크를 ELB 로 설정해야 합니다.

#### 오토 스케일링(Auto Scaling)
클라우드 컴퓨팅의 가장 대표적인 장점으로 소개되는 기능입니다. 필요에 따라 서비스를 위한 자원을 자동으로 확장하거나 축소하는 기능을 의미합니다. 기본적으로 CPU, 메모리, 디스크, 네트워크 트래픽을 모니터링하여 서버 자원에 대한 풀이나 사이즈를 자동을 조절합니다.

스케일링 방법에는 대표적으로 스케일업과 스케일아웃이 있습니다.

- **스케일 업**

  서버 내부의 자원의 사이즈를 조절하는 기능입니다. 예를 들어 CPU 의 수량을 늘리거나 그래픽카드를 좀 더 성능 좋은 모델로 변경하는 기능입니다. 단, 자원을 2배로 늘렸다고 해서 실제 성능이 2배로 늘어나는 것은 아닙니다.

- **스케일 아웃**

  서버의 수량 자체를 늘리는 기능입니다. 온프레미스환경에서의 공간의 제약이나 하드웨어적 제약이 없고 성능과 비용이 비례하기 때문에 클라우드 환경에서 탄력성있는 서비스를 구성하기 위해서는 반드시 필요한 기능입니다.
...
## 로그 추출 기능
로드 밸런서에서 처리한 요청 로그를 S3 로 추출할 수 있습니다. 장애 발생시 각 인스턴스 로그를 확인하는 것 보다 로드 밸런서의 로그를 확인하는 것이 좀 더 효율적입니다. CloudWatch 연동하여 좀 더 상세한 모니터링도 가능하다.
